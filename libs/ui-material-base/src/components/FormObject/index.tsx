...
class FormObject extends Component {
  ...
  render() {
    ...
    return (
      <div
        className={`sq-form-object ${className} ${
          this.state.fullScreen ? 'sq-form-object--full-screen' : ''
        }`}
      >
        <Dialog
          ...
          onAction={(data, dialogAction) =>
            this.handleAction(data, dialogAction)
          }
        ...
        >
          ...
          {this.state.currentError && (
            <Alert message={this.state.currentError} type="warning" />
          )}
        </Dialog>
        <div className="sq-form-object__top-actions mb-1">
          {!this.state.expandFull && label && (
            <IconButton
              className="sq-form-object__expand-collapse"
              iconSize="small"
              iconName="add-circle-outline"
              title="Expand"
              color="warning"
              size="small"
              onClick={() => this.setState({ expandFull: true })}
            />
          )}
          {this.state.expandFull && label && (
            <IconButton
              ...
              color="warning"
              size="small"
              onClick={() => this.setState({ expandFull: false })}
            />
          )}
          {label && <div className="sq-form-object__label">{label}</div>}
          <IconButton
            className={label ? '' : 'sq-form-object__float-full'}
            iconSize="small"
            iconName={this.state.fullScreen ? 'FullscreenExit' : 'Fullscreen'}
            onClick={this.toggleFullScreen}
          />
        </div>
        {this.state.expandFull &&
          value &&
                ...
                <div className="sq-form-object__item-wrap">
                  {isObject && !this.state.expandItems[itemKey] && (
                    <IconButton
                      ...
                      color="info"
                      size="small"
                      onClick={() => this.expandItem(itemKey)}
                    />
                  )}
                  {isObject && this.state.expandItems[itemKey] && (
                    <IconButton
                  ...
                  <Form
                    ...
                    fields={
                      ...
                      [
                        ...
                        (!isObject && this.state.expandFull) ||
                    ...
                    value={itemVal}
                    onChange={(data) =>
                      this.valueOnChange(data, itemKey, _isArray)
                    }
                  />
                  {!fields && !isObject && !isInternalArray && (
                    <IconButton
                      iconSize="small"
                      ...
                      color="success"
                      size="small"
                      onClick={() => this.convertToObj(itemKey, _isArray)}
                    />
                  )}
                  {!fields && !isObject && !isInternalArray && (
                    <IconButton
                      ...
                      color="success"
                      size="small"
                      onClick={() => this.convertToArr(itemKey, _isArray)}
                    />
                  )}
                  {!fields && !isObject && !isInternalArray && (
                    <IconButton
                      iconSize="small"
                      title={'Convert to bool'}
                      iconName="Code"
                      color="info"
                      size="small"
                      onClick={() => this.convertToBool(itemKey, _isArray)}
                    />
                  )}
                  <IconButton
                    iconSize="small"
                    ...
                    color="error"
                    size="small"
                    onClick={() => this.removeItem(itemKey, _isArray)}
                  />
                </div>
              </div>
            );
        ...
        <div className="sq-form-object__actions">
          <IconButton
            ...
            iconName="add"
            title={'Add'}
            onClick={() => this.addNew(_isArray)}
          />
          <IconButton
            iconSize="small"
            iconName="code"
            title={'Replace with Text'}
            onClick={this.replaceFromText}
          />
          {!_isArray && (
            <IconButton
              iconSize="small"
