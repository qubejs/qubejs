...
import { hooks, utils } from '@qubejs/web-react';
...
const SQAutocomplete = ({
  ...
  const handleChange = (e, _value) => {
    ...
    if (
      maxSelection &&
      Array.isArray(parkValue) &&
      parkValue.length > maxSelection
    ) {
      parkValue = [..._value];
    ...
    saveTextField &&
      ...
      onAction(
        {},
        {
          ...
          params: {
            [saveTextField]: multiple
              ? [
                  ...fixedOptions.map((i) => i[textField]),
                  ...parkValue
                    .filter((option) => fixedOptions.indexOf(option) === -1)
                    .map((i) => i[textField]),
                ]
              : parkValue && parkValue[textField],
          },
        }
      );
    onChange &&
      onChange({
        value: multiple
          ...
          : parkValue && parkValue[valueField],
        options,
      });
  };
  ...
  useEffect(() => {
    ...
    let finalOptions = [
      ...output,
      ...(multiple && preserveValues
        ? value
          ? value
              .filter((i) => !!i)
              .map((i) => ({ [textField]: i, [valueField]: i }))
          : []
        : []),
    ];
