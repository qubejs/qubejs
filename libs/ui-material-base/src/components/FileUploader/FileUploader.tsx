...
const SQFileUploader = ({
  ...
  const { Button, Icon, Alert, Progress, Input } =
  ...
  const handleChange = async (filenew) => {
    ...
    const listToBeAdded: any[] = [].filter.call(
      takeOne,
      (i: any) => file.filter((d) => d.name === i.name).length === 0
    );
  ...
  return (
    <div className={`sq-file-uploader ${className}`}>
      ...
      {enableLoader && progress && (
        <Progress className="tp-progress--active" style="default" />
      )}
      {file && (
      ...
      {uploadSuccessMessage && !progress && sucessCalled && (
        <Alert type={successMessageType} message={uploadSuccessMessage} />
      )}
      {uploadFailedMessage && !progress && failedCalled && (
        <Alert type={errorMessageType} message={uploadFailedMessage} />
      )}
    </div>
  );
};
