...
const renderPricingPlan = (data, onAnalytics) => {
  ...
  return (
    <div className={`sq-pricing__plan ${data.color}`}>
      <div className="sq-pricing__icon">
        {<Icon name={data.icon} color={data.iconVarant} variant="bg-big" />}
      </div>
          ...
          data.features.map((dataItem, idx) => {
            return (
              <>
                <div
                  key={idx}
                  className={`sq-pricing__feature-item ${dataItem.className}`}
                >
                  <Icon
                    name={dataItem.icon}
                    size="small"
                    variant={dataItem.iconColor || 'none'}
                  />
                  <span className="sq-pricing__feature-item-text">
                    {dataItem.text}
                  </span>
                </div>
              </>
  ...
  return (
    <div className="sq-pricing--default">
      <div className="sq-pricing__wrapper">
        {icon && <Icon name={icon} />}
        {eyebrow && (
          <div className={`sq-pricing__eyebrow`}>
            {ReactHtmlParser(eyebrow)}
          </div>
        )}
        {header && (
          <HTag className={`sq-pricing__header`}>
            {ReactHtmlParser(header)}
          </HTag>
        )}
        {subHeader && (
          <BTag className={`sq-pricing__sub-header mb-wide`}>
