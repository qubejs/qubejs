...
export const processParams = (userData, params = {}, defaultValue = null, state?):any => {
  let newObj :any= {};
...
export const fetchJsonPath = ({ url, params, headers }:any) => {
  const mode = win.getWindow().APP_CONFIG?.siteMode === 'static' ? 'get' : 'post';
  url = clearUrl(url);
  const { url: overrideUrl, params: overrideParams, method: overrideMethod } = extractUrlInfo(url, win.getWindow().APP_CONFIG?.siteMap?.siteMap?.dynamicContentConfig);
  const postFix = !overrideUrl && mode === 'get' ? '/get.json' : '';

  const cb = mode === 'get' ? { _cb: win.getWindow().APP_CONFIG?.appVersion } : {};
...
const getIfMatchPagesOnRoute = (path, errorCode) => {
  ...
  siteConfig && siteConfig?.siteMap?.errorRedirects &&
...
export const fetchContentPage = createAsyncThunk('content/fetchContentPage', async (url:any) => {
  ...
  if (response.error) {
    ...
    } else if (win.getWindow().APP_CONFIG?.siteMap.siteMap?.errorRedirects[response.code]) {
      response = await fetchJsonPath({
        url: win.getWindow().APP_CONFIG?.siteMap.siteMap?.errorRedirects[response.code],
      });
    }
  }
...
export const sendContact = createAsyncThunk('content/sendContact', async (data:any) => {
  ...
  if (response.status === 'success') {
    ...
    if (payload?.finally?.successAction) {
      if (Array.isArray(payload?.finally?.successAction)) {
        payload?.finally?.successAction.forEach((actionItem) => {
          ...
          isValid && events.emit('dynamicContent.onAction', {}, { ...actionItem, currentData }, {});
        });
      } else {
        events.emit('dynamicContent.onAction', {}, { ...payload?.finally?.successAction, currentData }, {});
      }
    }
    ...
    if (payload.finally?.errorAction) {
      if (Array.isArray(payload.finally.errorAction)) {
        ...
        events.emit('dynamicContent.onAction', {}, { ...payload?.finally?.errorAction, currentData }, {});
      }
    }
